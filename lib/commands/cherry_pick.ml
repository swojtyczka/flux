let cherry_pick (source : string) : unit =
  if not (Config.User.isset ()) then
    print_endline "Please set your name and email in ~/.fluxconfig"
  else
    let target =
    ...
    | Some hash -> (
        ...
        if List.length source_parents > 1 then
          print_endline "Can't cherry-pick merge commit"
        else
          match Internals.Head.get_current_commit () with
          | None -> print_endline "Can't cherry pick to empty branch!"
          | Some _ ->
              let base_index =
              ...
              else Commit.commit source_message)
