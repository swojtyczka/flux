let _ : unit =
  ...
  | "stage" -> if argc < 3 then print_endline "Too few arguments" else Flux.Commands.Stage.stage Sys.argv.(2)
  | "unstage" -> if argc < 3 then print_endline "Too few arguments" else Flux.Commands.Unstage.unstage Sys.argv.(2)
  ...
  | "delta" -> if argc < 3 then Flux.Commands.Delta.delta @@ Flux.Internals.Head.get_current_commit_hash () else Flux.Commands.Delta.delta Sys.argv.(2)
