let update_ref (ref : string) (commitHash : string) : unit =
  Out_channel.with_open_text (Filename.concat ".flux" ref) (fun oc -> Out_channel.output_string oc commitHash)

let list_all () : string list =
  Sys.readdir ".flux/refs/heads" |> Array.to_list

let get_current_commit (branch : string) : string =
  In_channel.with_open_bin (FilePath.concat ".flux/refs/heads" branch) In_channel.input_all 
