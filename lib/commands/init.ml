let init () : unit = 
  if Sys.file_exists ".flux"
  then print_endline "Repository already exists"
  else
    try 
    ...
    Yojson.to_file ".flux/HEAD" (`Assoc ["type", `String "ref"; "val", `String "refs/heads/master"]);

    print_endline "Initialized empty repository";

    with _ -> print_endline "Couldn't initalize repository. Does .flux already exist?"