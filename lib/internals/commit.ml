...
let get_commit_timeStamp_and_message (hash : string) : string * string =
  ...
  let timeStamp =
    Yojson.Basic.Util.to_string @@ Yojson.Basic.Util.member "timeStamp" commit
  in
  let message =
    Yojson.Basic.Util.to_string @@ Yojson.Basic.Util.member "message" commit
  in
  (timeStamp, message)

let get_commit_parent (hash : string) : string =
  if hash = "" || not (Sys.file_exists (Filename.concat ".flux/objects" hash))
  then ""
  else
    let commit =
      Yojson.Basic.from_file (Filename.concat ".flux/objects" hash)
    in
    Yojson.Basic.Util.to_string @@ Yojson.Basic.Util.member "parent" commit

let get_empty_commit_files () : Yojson.Basic.t = `List []

let get_commit_files (hash : string) : Yojson.Basic.t =
  if hash = "" || not (Sys.file_exists (Filename.concat ".flux/objects" hash))
  then get_empty_commit_files ()
  else
    let commit =
      Yojson.Basic.from_file (Filename.concat ".flux/objects" hash)
    in
